...
本前端已对接同级目录 Spring Boot 后端的 `/api/admin/**` 接口。开发环境通过 Vite 将 `/api` 代理到 `http://localhost:8080`，已开放以下模块：

- 管理员登录、JWT 认证与按权限动态菜单
- 商品、品牌、分类、属性、商品属性值与 SKU
- 订单查询、订单详情与发货
- 退款查询、审核通过与拒绝
- 会员查询、详情与启用/禁用
- 管理员、角色、资源及其分配关系
...
```bash
# 在后端根目录启动 Spring Boot
./mvnw spring-boot:run

# 在 mall-admin-web 目录启动前端
npm install
npm run dev
```

前端地址为 `http://localhost:5173`。生产构建使用 `npm run build`，生产环境默认请求同域 `/api`，应由网关或 Nginx 转发到 Spring Boot。
