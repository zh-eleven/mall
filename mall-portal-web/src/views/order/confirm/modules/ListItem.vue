...
import { previewOrder, submit } from '@/api/order'
...
const loadGoodsList = async () => {
  ...
  if (!addressId || loadingGoods.value) {
    return
  }
  ...
  try {
    const res = await previewOrder(addressId)
    const preview = res.data || {}

    let items = (preview.items || []).map(item => ({
      id: item.cartItemId,
    ...
    goodsAmount.value = Number(preview.totalAmount || 0)
    orderTotalAmount.value = Number(preview.payAmount || 0)
    freightPrice.value = 0
  } catch (error) {
    showToast({
      type: 'fail',
      message: error?.message || '订单预览加载失败'
    })
  } finally {
...
watch(
  ...
  addressId => {
    if (addressId && selectedCartKey.value) {
      loadGoodsList()
    }
  },
  {
