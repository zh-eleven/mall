...
Mall MVP 的移动端 H5 商城前台，基于 `waynboot-mobile` 改造，负责首页、分类、搜索、商品详情、购物车、下单、订单、地址和用户中心等购物链路。

项目已经迁移到 Vue 3 + Vite 6 + Vant 4。当前代码重点是移动端体验、浅色 Apple 风格 UI、清晰商品信息和稳定下单流程。

## 当前后端接入范围

已经与同仓库 Spring Boot 后端打通会员注册/登录、商品分类与详情、购物车、
收货地址、订单预览/提交/查询/取消/测试支付/确认收货，以及退款申请。

优惠券、秒杀、评论、真实支付和部分首页运营接口仍属于后续迭代范围；前端中保留的
相关页面或交互不代表当前 MVP 已具备对应生产能力。项目整体启动方式见
...
本目录保留上游 MIT License，详见 [LICENSE](LICENSE)。
